...
async function geminiExtractQuestionsFromPDF(pdfPath: string): Promise<ParsedQuestion[]> {
  ...
  const prompt = `You are reading a multi-page MCQ exam PDF. Extract ONLY the multiple-choice
questions that appear BEFORE any solutions/explanations.

Rules:
- Each question has exactly four options (a/b/c/d). Normalize option keys to A,B,C,D.
- If an "Answer Key" table exists (e.g., "1.b 2.b 3.d ..."), use it to set the correctAnswer for each question number.
...
- Do NOT include explanations or solution steps.`;
  ...
  let parsed: any = {};
  try { parsed = JSON.parse(jsonText || "{}"); } catch { parsed = { questions: [] }; }
...
export async function extractQuestionsFromPDF(pdfPath: string): Promise<ParsedQuestion[]> {
  ...
  if (!questions.length) {
    try {
      ...
      if (!questions.length) console.warn(`No questions parsed from ${path.basename(pdfPath)}.`);
    } catch (err) {
